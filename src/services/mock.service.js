...
class MockService {
  ...
  previewUsers(count, role) {
    const n = this.***REMOVED***clamp(count, 10, MAX_PREVIEW_COUNT);

    if (role && !Object.values(ROLES).includes(role)) {
      throw new ApiError(400, `Rol invalido. Valores permitidos: ${Object.values(ROLES).join(', ')}`);
    }
  ...
  previewFullDataset({ users, orders, deliveries } = {}) {
    const usersCount = this.***REMOVED***clamp(users, 5, MAX_PREVIEW_COUNT);
    const ordersCount = this.***REMOVED***clamp(orders, 5, MAX_PREVIEW_COUNT);
    const deliveriesCount = this.***REMOVED***clamp(deliveries, 5, MAX_PREVIEW_COUNT);
  ...
  async seedDatabase({ users, orders, deliveries } = {}) {
    const usersCount = this.***REMOVED***clamp(users, 10, MAX_SEED_COUNT);
    const ordersCount = this.***REMOVED***clamp(orders, 10, MAX_SEED_COUNT);
    const deliveriesCount = this.***REMOVED***clamp(deliveries, 5, MAX_SEED_COUNT);
  ...
  async ***REMOVED***seedOrders(count, createdUsers, warnings) {
    ...
    if (customers.length === 0) {
      throw new ApiError(
        400,
        'No hay usuarios con rol "customer" para asociar a los pedidos. Generá usuarios primero (parametro "users" > 0).'
      );
    }
  ...
  async ***REMOVED***seedDeliveries(count, createdUsers, createdOrders, warnings) {
    ...
    if (drivers.length === 0) {
      warnings.push(
        'No se crearon entregas: no hay usuarios con rol "driver" disponibles. Generá usuarios primero (parametro "users" > 0).'
      );
      return [];
    }
   ...
   * [0, max]. Si no se especifica, usa el default.
   */
  ***REMOVED***clamp(value, defaultValue, max) {
    if (value === undefined || value === null || value === '') {
      return defaultValue;
    }
    const n = Number(value);
    if (!Number.isFinite(n) || n < 0) {
      throw new ApiError(400, 'La cantidad solicitada debe ser un numero mayor o igual a 0');
    }
